...
type IDatabaseFunctions =
    abstract member LoadArticles : Domain.UserData -> Domain.ArticleAssignment -> Task<Domain.ArticleList>
    ...
    abstract member UserPassedTraining : Domain.UserData -> Task<bool>
...
let getDatabase databaseType startupTime =
    ...
    | DatabaseType.AzureStorage connection ->
        ...
        { new IDatabaseFunctions with
            member __.LoadArticles user articleType = Storage.AzureStorage.getArticlesFromDB connection user articleType
            ...
            member __.UserPassedTraining user = Storage.AzureStorage.UserPassedTraining connection user
        }

    | DatabaseType.FileSystem ->
        { new IDatabaseFunctions with
            member __.LoadArticles user articleType = task { return Storage.FileSystem.getArticlesFromDB user articleType }
            ...
            member __.UserPassedTraining user = task { return false }
        }
